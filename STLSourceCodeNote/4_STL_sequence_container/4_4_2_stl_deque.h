...
template <class _Tp, class _Ref, class _Ptr> struct _Deque_iterator {
  ...
  _Tp *_M_last;
  _Map_pointer
      _M_node; // 指向map，map是连续空间，每个元素都是指针指向一块缓冲区
...
protected:
  typedef
      typename _Alloc_traits<_Tp *, _Alloc>::allocator_type _Map_allocator_type;
...
template <class _Tp, class _Alloc> class _Deque_alloc_base<_Tp, _Alloc, true> {
public:
  typedef typename _Alloc_traits<_Tp, _Alloc>::allocator_type allocator_type;
...
template <class _Tp, class _Alloc> _Deque_base<_Tp, _Alloc>::~_Deque_base() {
  if (_M_map) {
