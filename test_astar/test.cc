...
struct Node {
  ...
  Node(int x, int y, int cost, int estimate)
      : x(x), y(y), cost(cost), estimate(estimate) {}
...
vector<pair<int, int>> a_star(vector<vector<int>> &map, int startX, int startY,
...
// 清扫所有垃圾的主函数
vector<pair<int, int>> clean_all_trash(vector<vector<int>> &map, int startX,
                                       int startY) {
  ...
  while (true) {
    ...
    // 找到最近的垃圾
    vector<pair<int, int>> path =
        a_star(map, currentX, currentY, trashX, trashY);
...
int main() {
  // 初始化地图
  vector<vector<int>> map = {{0, 0, 1, 0, 2},
                             {0, 1, 0, 0, 1},
                             {0, 0, 0, 2, 0},
                             {1, 0, 1, 0, 0},
                             {0, 0, 0, 1, 0}};
