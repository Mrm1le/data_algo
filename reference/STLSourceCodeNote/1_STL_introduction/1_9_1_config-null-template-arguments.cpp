...
template <class T, class Sequence = deque<T>> class stack {
  ...
  // has already been declared and add <> after the function name here) friend
  // bool operator==  (const stack &, const stack &); friend bool operator<
...
int main() {
  ...
  stack<char> y1;
  // error: no match for 'operator==' (operand types are 'stack<int>' and
  // 'stack<char>') cout << (x == y1) << endl;
}