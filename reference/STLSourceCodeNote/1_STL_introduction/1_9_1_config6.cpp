...
template <class T, class Alloc = alloc> class vec {
public:
  void swap(vec<T, Alloc> &) { cout << "swap()" << endl; }
};

// inline内联函数，解决一些频繁调用的小函数大量消耗栈空间（栈内存）的问题
// inline只适合函数体内代码简单的函数使用，不能包含复杂的结构控制语句例如
// while、switch，并且不能内联函数本身不能是直接递归函数
