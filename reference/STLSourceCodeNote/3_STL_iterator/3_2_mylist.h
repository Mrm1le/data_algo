...
template <typename T> void List<T>::insert_front(T value) {
...
template <typename T> void List<T>::insert_end(T value) {
  ListItem<T> *temp = new ListItem<T>(value);
...
// 声明写了默认参数后定义不能重写
template <typename T> void List<T>::display(std::ostream &os) const {
